...
//for checking the user (updated 2024)
function checkUser($login_data) {
    ...
    if (count($data['user']) > 0) {
        $data['status'] = true;
        // Update activity status to 1 for the user
        $query_update_activity = "UPDATE users SET activity = 1 WHERE email='$username_email' OR username='$username_email'";
        mysqli_query($db, $query_update_activity);
    } else {
        $data['status'] = false;
    }
...
//updated 2024 for active users
function filterActiveUser() {
    $list_cht = getFollowSuggestions();
    $filtered_usrs = array();
    foreach ($list_cht as $ssuser) {
        if (checkFollowStatus($ssuser['id']) && !checkBS($ssuser['id'])) {
            $user_data = getUser($ssuser['id']);
            if ($user_data && $user_data['activity'] == 1) {
                $filtered_usrs[] = $ssuser;
            }
        }
    }
    return $filtered_usrs;
}
...
function createUser($data){
 ...
 $query.="VALUES ('$first_name','$last_name',$gender,'$email','$username','$password',0)"; 
 return mysqli_query($db,$query);
}
...
function validateUpdateForm($form_data,$image_data){
    ...
    function updateProfile($data,$imagedata,$cover){
        global $db;
        $first_name = mysqli_real_escape_string($db,$data['first_name']);
        $last_name = mysqli_real_escape_string($db,$data['last_name']);
        $username = mysqli_real_escape_string($db,$data['username']);
...
$cover_pic="";
if($cover['name']){
    $image_name_c = time().basename($cover['name']);
    $image_dir_c="../images/cover/$image_name_c";
    move_uploaded_file($cover['tmp_name'],$image_dir_c);
    $cover_pic=", cover_pic='$image_name_c'";
}
       
      
    
        $query = "UPDATE users SET first_name = '$first_name', last_name='$last_name',username='$username',password='$password' $profile_pic $cover_pic WHERE id=".$_SESSION['userdata']['id'];
return mysqli_query($db,$query);
...
function validatePostImage($image_data){
    ...
    $response['status']=true;
       if($image_data['name']){
        ...
        if($size>5000){
            $response['msg']="upload image less then 5MB";
            $response['status']=false;
            $response['field']='post_img';
        }
       }
...
$user_id = $_SESSION['userdata']['id'];
if($image==null){
    $image_name = "000icon.png";
}else{
        $image_name = time().basename($image['name']);
       
}
if($post_text!=null || $image_name!=null){
