...
if(isset($_GET['logout'])) {
    if(isset($_SESSION['userdata']['id'])) {
        ...
        $query = "UPDATE users SET activity = 0 WHERE id = '$user_id'";
        $result = mysqli_query($db, $query);
        if(!$result) {
            echo "Failed to update user's activity status.";
            exit(); 
        }
    }
    session_destroy();
...
if(isset($_GET['updateprofile'])){
    ...
    $response=validateUpdateForm($_POST,$_FILES['profile_pic'],$_FILES['cover_pic']);

    if($response['status']){
       
        if(updateProfile($_POST,$_FILES['profile_pic'],$_FILES['cover_pic'])){
