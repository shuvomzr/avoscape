...
?>
    ...
    <div class="col-12 rounded p-4 mt-4 d-md-flex gap-5" style="background: url('assets/images/cover/<?=$profile['cover_pic']?>') no-repeat; background-size: cover;height:360px;">
            ...
            <div class="col-md-8 col-sm-11">
                <div class="d-flex flex-column">
                    <div class="d-flex gap-5 align-items-center">
                        <span style="font-size: xx-large; color:#fff;font-weight: bold;text-shadow:1px black;"><?=$profile['first_name']?> <?=$profile['last_name']?></span>
...
if($user['id']!=$profile['id'] && !checkBS($profile['id'])){
  ...
  <div class="dropdown">
                            <span class="" style="font-size:xx-large" type="button" id="dropdownMenuButton1"
                                data-bs-toggle="dropdown" aria-expanded="false"><i class="text-light bi bi-three-dots"></i> </span>
    ...
    <?php
                    ...
                    <span style="font-size: larger;" class="text-light">@<?=$profile['username']?></span>
                    <?php
...
if($user['id']==$profile['id']){
    ?>
  <a class="btn btn-sm btn-primary" href="#" data-bs-toggle="modal" data-bs-target="#chatbox" onclick="popchat(<?=$profile['id']?>)"><i class="bi bi-chat-fill"></i> Private Message</a>
    
    <?php
}else{
?>
<a class="btn btn-sm btn-primary" href="#" data-bs-toggle="modal" data-bs-target="#chatbox" onclick="popchat(<?=$profile['id']?>)"><i class="bi bi-chat-fill"></i> Message</a>
...
?> 
<button class="btn btn-sm text-danger btn-primary unblockbtn" style="width:200px;" data-user-id='<?=$profile['id']?>' >Unblock</button>
...
<?php
   ...
   ?>
<button class="btn btn-sm btn-primary unfollowbtn text-danger" data-user-id='<?=$profile['id']?>'><i class="bi bi-person-x-fill"></i> Unfollow</button>
   
   <?php
}else{
    ?>
<button class="btn btn-sm btn-primary followbtn text-primary" data-user-id='<?=$profile['id']?>' ><i class="bi bi-person-plus-fill"></i> Follow</button>
...
?>
        ...
        <h3 class="border-bottom p-3">List of Post</h3>
        <?php
...
if(checkBS($profile['id'])){
 ...
 <div class="alert bg-danger text-center" role="alert">
   ...
   <?php
    ...
    echo "<p class='p-2 bg-danger border rounded text-center my-3'>Nothing is Posted in this Profile</p>";
}
        ?>
...
foreach($profile_post as $post){
    ...
    ?>
            <img src="assets/images/posts/<?=$post['post_img']?>" data-bs-toggle="modal" data-bs-target="#postview<?=$post['id']?>" width="310px" class="rounded" />
...
<button type="button" class="btn-close btn-sm" data-bs-dismiss="modal" aria-label="Close" style="margin-top:0px!important;padding:0px!important;color:red!important;"></button>
  <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1" style="background: linear-gradient(135deg, #71b7e6, #3ca756);">
  <?php
...
<div class="d-flex align-items-center p-2">
  ...
  <div class="input-group p-2 border-top">
                            <input type="text" class="form-control rounded-left border-0 comment-input" placeholder="Comment here..."
                                aria-label="Recipient's username" aria-describedby="button-addon2">
                            <button class="btn btn-primary rounded-right border-right add-comment" data-cs="comment-section<?=$post['id']?>" data-post-id="<?=$post['id']?>" type="button"
                                id="button-addon2">Comment</button>
                        </div>
                            <?php
                        }else{
                            ?>
<div class="text-center p-2"> Only flowers can comment here</div>
...
foreach($profile['followers'] as $f){
    ...
    if(checkFollowStatus($f['follower_id'])){
        $fbtn = '<button class="btn btn-sm btn-danger border unfollowbtn" data-user-id='.$fuser['id'].' >Unfollow</button>';
