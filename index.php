...
if(isset($_SESSION['Auth'])){
    ...
    $activeUserList = filterActiveUser();
}
...
if(isset($_SESSION['Auth']) && $user['ac_status']==1 && !$pagecount){
    ...
    showPage('header',['page_title'=>'Follwers Suggestions for you']);
    showPage('navbar');
    showPage('followers');
}elseif(isset($_SESSION['Auth']) && isset($_GET['shortVid']) && $user['ac_status']==1){
    showPage('header',['page_title'=>'Short video']);
    showPage('navbar');
    showPage('shortVid');
}
elseif(isset($_SESSION['Auth']) && isset($_GET['active_list']) && $user['ac_status']==1){
    showPage('header',['page_title'=>'Active people list']);
    showPage('navbar');
    showPage('active_list');
}
elseif(isset($_SESSION['Auth']) && isset($_GET['u']) && $user['ac_status']==1){
    ...
    showPage('header',['page_title'=>'avoScape - SignUp']);
    showPage('signup');
}elseif(isset($_GET['login'])){
   
    showPage('header',['page_title'=>'avoScape - Login']);
    showPage('login');
}elseif(isset($_GET['forgotpassword'])){
    
    showPage('header',['page_title'=>'avoScape - Forgot Password']);
